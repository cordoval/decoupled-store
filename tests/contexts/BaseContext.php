...
class BaseContext implements ContextInterface
{
    ...
    public function customerIsSentToPaypalSite()
    {
        ...
        ;
        expect($this->helper->redirectsCustomerToThirdPartyForPayment())
            ->toBeAnInstanceOf('Symfony\Component\HttpFoundation\RedirectResponse')
        ;
        expect($this->helper->waitsForResponseFromThirdPartyAfterPaymentIsDone())->toBe(['confirmed' => true]);
        expect($this->helper->issueOrderAndSendBookToCustomerUponConfirmation())->toBe(true);
    }
